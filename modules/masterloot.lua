...
function ShowDialog(unitPlayer, itemAward)
	DLG:Spawn("EPGP_CONFIRM_GP_CREDIT", {name = unitPlayer:GetName(), icon = itemAward })
end
	...
	if tMLoot.wndMasterLoot ~= nil then
		for idx, wndLooter in pairs(tMLoot.wndMasterLoot:FindChild("LooterList"):GetChildren()) do
			...
			if nEP >= EPGP.db.profile.nMinEP then
				local wndOverlay = Apollo.LoadForm(EPGP.xmlDoc, "EPGPOverlay", wndLooter, tEPGP)
